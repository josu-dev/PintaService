...
async def dev_server(cli: argparse.Namespace):
    def live_reload_coroutine():
        ...
        host = (
            LRWS_HOST if cli.live_reload_host is None else cli.live_reload_host
        )
        port = (
            LRWS_PORT if cli.live_reload_port is None else cli.live_reload_port
        )
    ...
    def flask_server_executor(
        ...
        host_arg = (
            "--host" + cli.flask_host if cli.flask_host is not None else ""
        )

        port_arg = (
            "--port" + cli.flask_port if cli.flask_port is not None else ""
        )
...
def minify_tailwindcss(cli: argparse.Namespace):
    ...
    dev_print("Minifying tailwindcss for production...")
